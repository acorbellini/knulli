...
# Build RK3566-BSP (Miyoo Flip v2 and other RK3566 devices) on a
# remote Fedora Linux machine via SSH. Syncs the source tree with rsync,
# builds natively on the remote host, and can copy artifacts back.
#
# Usage: ./build-rk3566-bsp-remote.sh [command]
# Examples:
#   ./build-rk3566-bsp-remote.sh              # sync source + full build
#   ./build-rk3566-bsp-remote.sh shell        # open an SSH shell in the build dir
#   ./build-rk3566-bsp-remote.sh copy         # copy build artifacts to local host
#   ./build-rk3566-bsp-remote.sh clean        # remove remote build output
#   ./build-rk3566-bsp-remote.sh deps         # install build dependencies on remote
#   ./build-rk3566-bsp-remote.sh sync         # sync source only (no build)
...
BUILD_EOF
    ...
    shell)
        ...
        ;;
    copy)
        echo "==> Copying miyoo-flip images to ${OUTPUT_DIR}/${TARGET}/..."
    ...
    clean)
        ...
        echo "==> Done. Run './build-rk3566-bsp-remote.sh' to start a fresh build."
        ;;
    *)
        sync_source
        NPROC=$(get_jobs)
